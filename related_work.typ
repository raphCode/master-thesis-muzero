...
effzero by #citet("efficientzero") is a modification of muzero to achieve similar
performance like the original algo, but requiring less training data.
...
#[
  ...
  This is the idea behind effzero's similarity loss, which #citet("efficientzero") call
  #sscl.
  ...
  and therefore using it as a target for the dnet.~@efficientzero
...
the state aliasing problem.~@efficientzero
...
with a LSTM architecture.~@efficientzero
...
#citet("efficientzero") propose to only use rews over a smaller horizon, with the size
...
therefore considered a more accurate value.~@efficientzero
